...
{
    uint64_t dest_ext = sign_ext_64(dest& (0xFFFFFFFF >> (32 - data_size)));
    uint64_t src_ext = sign_ext_64(src& (0xFFFFFFFF >> (32 - data_size))); 
...
uint32_t alu_and(uint32_t src, uint32_t dest, size_t data_size)//pass
{
    uint32_t res=0;
    res=dest&src;
    cpu.eflags.OF=0;
...
uint32_t alu_xor(uint32_t src, uint32_t dest, size_t data_size)//pass
{
    uint32_t res=0;
    res=dest^src;
    cpu.eflags.OF=0;
...
uint32_t alu_or(uint32_t src, uint32_t dest, size_t data_size)//pass
{
    uint32_t res=0;
    res=dest|src;
    cpu.eflags.OF=0;
